    ...
    <div className='z-50 flex flex-col fixed w-full'>
      ...
      {ANNOUNCEMENT_TEXT ? (
        <motion.div
          ...
          initial={{ opacity: 0, translateY: '-100%', position: 'static' }}
          animate={
            !announceDismissed
              ? { opacity: 1, translateY: '0%' }
              : { opacity: 0, translateY: '-100%', position: 'absolute' }
          }
          transition={{ duration: 1, ease: 'easeOut' }}
        >
          <p className='w-full font-semibold text-md text-center text-orange-200'>
            {ANNOUNCEMENT_TEXT}
          </p>
          <Image
            width={1920}
      ...
      {/* Top Bar */}
      <motion.div
        className='z-20 flex relative h-[80px] justify-between items-center bg-[#1d1d1d00] text-gray-300'
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
