...
const LeaderCard: React.FC<props_LeaderCard> = ({ leaderName, index }) => {
    ...
    return (
        <motion.div ref={ref} onAnimationStart={() => {setIsAnimating(true)}} onAnimationComplete={() => {setIsAnimating(false)}} onMouseEnter={() => playSfx_hoverThunk()}
            ...
            className={`${isAnimating && "pointer-events-none"} hover:z-20 outline-2 outline-green-950 outline relative group opacity-0 hover:rounded-b-none hover:!scale-105 transition duration-300 ease-out select-none rounded-xl bg-gradient-to-t from-[#040404] via-lightpall to-medpall`} key={leaderName}>
            ...
            <div className="hover:scale-105 active:scale-95 transition duration-300 ease-out" onMouseDown={() => {
                ...
                <motion.div animate={displayQuote ? { opacity: 1 } : { opacity: 0 }} className="z-10 absolute w-full outline-4 active:outline-8 active:outline-neutral-100 duration-200 outline-double opacity-0 outline-black bg-gradient-to-b from-[#5a512970] to-black rounded-xl drop-shadow-2xl">
