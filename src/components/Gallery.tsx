...
const imageClasses =
  'relative max-h-[100%] min-w-[100%] object-cover duration-500 hover:!opacity-100 group-hover:opacity-70' // hover:object-contain
const listItemClasses =
  'flex-grow p-0 w-[33vw] h-[26vh] hover:z-10 group/inner duration-500 hover:!z-10 active:!scale-[130%] hover:!scale-110'
...
const GalleryPiece: React.FC<props_GalleryPiece> = ({
  pieceDetails,
  index,
}) => {
  ...
  return (
    <motion.div
      ...
      animate={
        isInView ? { opacity: 1, transform: 'scale(1) translateY(0%)' } : {}
      }
      transition={{ duration: 1, delay: index * 0.2, ease: 'easeInOut' }}
      className={listItemClasses + (isAnimating ? ' pointer-events-none' : '')}
    >
      <p className='z-20 group-hover/inner:opacity-100 opacity-0 absolute bottom-0 w-full text-center duration-300 font-semibold text-shadow-sm shadow-black bg-[#00000050]'>
        {pieceDetails.remark}
      </p>
      <Image
    ...
    <div className='w-full bg-gradient-to-b from-slate-900 via-black to-black text-gray-300'>
      ...
      <div className='py-[8vh] overflow-hidden'>
        <div className='flex flex-wrap w-full group'>
          {gallery.map((pieceDetails, index) => (
            <GalleryPiece
