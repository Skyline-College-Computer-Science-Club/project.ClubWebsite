...
function GalleryPiece({ pieceDetails, index }: { pieceDetails: GalleryPiece, index: number }): React.ReactNode {
...
export default function Gallery({ collection }: { collection: Array<GalleryPiece> }) : React.ReactNode {
    return (
        <div className="py-20 overflow-hidden">
            <div className="flex flex-wrap w-full group">
                {collection.map((pieceDetails, index) => (<GalleryPiece key={pieceDetails.toString()} pieceDetails={pieceDetails} index={index}/>))}
            </div>
        </div>
    )
}