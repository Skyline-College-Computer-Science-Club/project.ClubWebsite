...
const LeaderCard: React.FC<props_LeaderCard> = ({ leaderName, index }) => {
    ...
    return (
        <motion.div ref={ref} onAnimationStart={() => {setIsAnimating(true)}} onAnimationComplete={() => {setIsAnimating(false)}} onMouseEnter={() => playSfx_hoverThunk()}
            ...
            className={`${isAnimating && "pointer-events-none"} hover:z-20 outline-2 outline-[#3c910b] outline relative group opacity-0 hover:rounded-b-none hover:!scale-105 transition duration-300 ease-out select-none rounded-xl bg-gradient-to-t from-[#040404] via-[#64c72b] to-[#1f3f0d]`} key={leaderName}>
