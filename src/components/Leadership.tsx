...
const LeaderCard: React.FC<props_LeaderCard> = ({ leaderName, index }) => {
  ...
  return (
    <motion.div
      ...
      animate={isInView ? { opacity: 1, transform: 'translateX(0%)' } : ''}
      transition={{ duration: 0.6, delay: 0.2 + index / 8, ease: 'easeOut' }}
      className={`${isAnimating && 'pointer-events-none'} hover:z-20 relative group opacity-0 hover:rounded-b-none hover:!scale-105 transition duration-300 ease-out select-none rounded-xl bg-gradient-to-t from-[#040404] via-green-950 to-green-950`}
      key={leaderName}
    >
      ...
      >
        <motion.div
          ...
          className='z-10 absolute outline-4 active:outline-8 active:outline-neutral-100 duration-200 outline-double opacity-0 outline-black bg-gradient-to-b from-[#040a0470] to-black rounded-xl drop-shadow-2xl'
        >
          <motion.div
            className='absolute'
          ...
          <div className='aspect-square w-full flex items-center px-10 font-semibold text-shadow-lg shadow-black'>
            ...
            <div className='text-center whitespace-pre-wrap'>
              {leaderDetails.quote ? leaderDetails.quote : FALLBACK_QUOTE}
            </div>
          </div>
        </motion.div>
      ...
      <div
        ...
        className='z-30 absolute hidden group-hover:flex justify-center w-full rounded-b-xl bg-gradient-to-b from-[#040a04] to-black gap-2 px-2 pb-2'
      >
